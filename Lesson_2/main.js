...
function main() {
  ...
  var AngleX = 0;
	...
	var map = [];
		for (var x_map = 0; x_map < 1000; x_map++) {
			map[x_map] = [];
			for (var y_map = 0; y_map < 100; y_map++) {
				map[x_map][y_map] = [];
				for (var z_map = 0; z_map < 1000; z_map++) {
					map[x_map][y_map][z_map] = 0;
				}
			}
		}

		for (var x_map = 0; x_map < 64; x_map++) {
			for (var y_map = 0; y_map < 3; y_map++) {
				for (var z_map = 0; z_map < 64; z_map++) {
					map[x_map][y_map][z_map] = 1;
				}
			}
		}
  ...
  function drawScene(time) {
    ...
    for (var x_map = 0; x_map < 64; ++x_map) {
		for (var y_map = 0; y_map < 100; ++y_map) {
			for (var z_map = 0; z_map < 64; ++z_map) {
				if (map[x_map][y_map][z_map] == true) {
					var draw = false;
					if(x_map == 0 || y_map == 0 || z_map == 0 || x_map == x_size - 1 || y_map == y_size - 1 || z_map == z_size - 1) draw = true;
					else if (map[x_map + 1][y_map][z_map] == false || map[x_map - 1][y_map][z_map] == false 
						|| map[x_map][y_map + 1][z_map] == false || map[x_map][y_map - 1][z_map] == false
						|| map[x_map][y_map][z_map + 1] == false || map[x_map][y_map][z_map - 1] == false) draw = true;
					if(draw){
						var matrix = computeMatrix(
							viewProjectionMatrix,
							[x_map*block_size*2, y_map*block_size*2, z_map*block_size*2],
							0,
							0);
